...
2. `common/CLAUDE-worker.md` — worker 넷의 규칙 (통신·인계·보고 서식)
...
6. `common/hooks/session-start.js` — 세션 시작·압축 뒤 되읽기
...
루트/
  crew/                  이 저장소. 봇이 여기서 뜬다: cwd = crew/bots/<봇>/
    ...
    bots/<봇>/           CLAUDE.md · 기억.md · .claude/settings.json(생성) · .env(토큰, 생성)
    proposals/           개선 제안 (제안 하나 = 파일 하나)
    setup.js             설치 스크립트
  rooms/<과제>/          작업장. 과제마다 git. orchestrator 가 open-room 으로 만든다
  knowledge/             회사 지식. domain/(archivist) · projects/(orchestrator). git 하나
```
...
- `@기억.md` import가 자동 압축 뒤에도 사는가
- `session-start.js` 가 `현재방` 파일로 방별 인계·작업상태를 되읽는가
