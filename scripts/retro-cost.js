...
//
//   node scripts/retro-cost.js [--since YYYY-MM-DD] [--room <방번호>]
//
...
console.log(`\n비용 계측 · ${sinceLabel} 이후 · ${new Date().toISOString().slice(0, 10)}\n`);
  ...
  try {
    ...
    for (;;) {
      ...
      for (const m of messages) {
        total++;
        const b = m.body || '';
        if (b.includes('승인을 요청')) { req++; pending = new Date(m.created_at).getTime(); }
        else if (b.includes('승인 전송됨') && pending) { wait += (new Date(m.created_at).getTime() - pending) / 60000; pending = null; }
      }
      after = messages[messages.length - 1].id;
    }
    console.log(`\n  도구 승인      사람이 누른 승인 요청 ${req}회 · 누적 대기 ${wait.toFixed(1)}분 · 방 메시지 ${total}건`);
