...
- 통과·접수·대기 통지에는 답하지 않는다. 다음 배분을 기다린다.
  되물을 것이 있을 때만 답하고, 그때도 무엇을 묻는지 한 줄로 쓴다.
...
## 분량
- 공통 규칙의 "분량" 절을 따른다. 나는 orchestrator 에게만 말하므로 내 상한은 언제나 600자·6줄이다.
- 보고는 이미 task-N-report.md 에 있다. 보내기 전에 그 파일을 세고, 나온 줄을 그대로 글 끝에 붙인다.

      node 루트/crew/scripts/count.js 루트/rooms/<방>/<나>/task-N-report.md
...
결과물: 경로
- 칸이 좁다고 조사와 어미를 지우지 않는다. 넘칠 것 같으면 수치를 빼고 파일 경로를 남긴다.
- "근거" 칸에는 무엇으로 확인했고 결과가 어땠는지만 쓴다. 개별 수치·해시·커밋 번호는 근거 파일에 두고 경로만 적는다.
